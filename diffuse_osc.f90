...
parameter(betaxu=1.06d0,betaxd=1.06d0,betay=1.06d0,fr=1.d0)
parameter(Re=100.d0,rad_factor=1.8d0,pi=4.d0*datan(1.d0))
parameter(N1=31,N2=194,M1=44,M2=207,startingtime=0.d0)
parameter(N=300,M=M1+M2-1,Ng=3,lspace=10)
parameter(Amp=0.25d0,St0=0.172d0)
...
!        if(icomput(i,j).ge.1)then
     ...
     if(l.eq.2)ru(l,i,j)=(1.d0-phiv(i,j))*ru(l,i,j)+phiv(i,j)*vely(i,j)
       if(l.eq.1)then
 	ru(l,i,j)=ru(l,i,j)-(1.d0-phiv(i,j))*pseudo_facx*dt*0.5d0
 	endif
 	
         if(l.eq.2)then
 	ru(l,i,j)=ru(l,i,j)-(1.d0-phiv(i,j))*pseudo_fac*dt*0.5d0
 	endif
